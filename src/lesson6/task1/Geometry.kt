...
data class Circle(val center: Point, val radius: Double) {
    ...
    fun distance(other: Circle): Double {
        when {
            center.distance(other.center) <= radius + other.radius -> return 0.0
...
/**
    ...
    fun circleByDiameter(diameter: Segment): Circle {
        ...
        val x = when {
            diameter.begin.x > diameter.end.x -> diameter.end.x + ((diameter.begin.x - diameter.end.x) / 2.0)
            else -> diameter.begin.x + ((diameter.end.x - diameter.begin.x) / 2.0)
        }
        val y = when {
            diameter.begin.y > diameter.end.y -> diameter.end.y + ((diameter.begin.y - diameter.end.y) / 2.0)
